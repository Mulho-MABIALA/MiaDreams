...
HTML;
                    ...
                    return '<link rel="stylesheet" href="' . asset('css/admin.css') . '?v=22">';
                }
            )
...
<script>
(() => {
    const hideDashboardModals = () => {
        if (!document.querySelector('.fi-dashboard-page')) return;

        document
            .querySelectorAll('.fi-modal, [aria-modal="true"], form[wire\\:submit*="callMountedAction"], form[wire\\:submit\\.prevent*="callMountedAction"]')
            .forEach((element) => {
                if (element.dataset.miaDashboardHidden === '1') return;
                element.dataset.miaDashboardHidden = '1';
    ...
    hideDashboardModals();
    requestAnimationFrame(hideDashboardModals);
    document.addEventListener('livewire:navigated', hideDashboardModals);

    new MutationObserver(hideDashboardModals).observe(document.body, {
        childList: true,
        subtree: true,
        attributes: true,
