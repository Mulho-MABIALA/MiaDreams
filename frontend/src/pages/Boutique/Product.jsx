    ...
    if (!product) return (
        <Layout title="Produit introuvable">
            <div className="min-h-screen bg-[#050505] flex flex-col items-center justify-center gap-5 px-6">
                ...
                <p className="font-glacial text-white/50 tracking-[3px] uppercase text-sm">{t('boutique_empty')}</p>
            ...
            <div className="bg-[#050505] min-h-screen pt-20 pb-12 sm:pt-24 sm:pb-20 px-4 sm:px-6 lg:px-10">
                <div className="max-w-6xl mx-auto">
                    ...
                    <nav className="flex items-center gap-2 mb-10 flex-wrap">
                        <Link to="/"        className="font-lastica text-[8px] tracking-[2px] text-white/40 hover:text-white/70 transition-colors uppercase">{t('nav_home')}</Link>
                        <span className="text-white/20">/</span>
                        <Link to="/boutique" className="font-lastica text-[8px] tracking-[2px] text-white/40 hover:text-white/70 transition-colors uppercase">{t('nav_boutique')}</Link>
                        <span className="text-white/20">/</span>
                        ...
                        <div className="flex flex-col">
                            ...
                            <div className="flex items-baseline gap-4 mb-5 pb-5 border-b border-white/[0.08]">
                                ...
                                {product.compare_price > product.price && (
                                    <span className="font-glacial text-base sm:text-lg text-white/60 line-through">
                                        {product.compare_price.toLocaleString('fr-FR')} FCFA
                                    </span>
                                )}
                            </div>

                            {/* Description — traduite dynamiquement */}
                            {product.description && (
                                <TranslatedText text={product.description} as="p"
                                    className="font-glacial text-sm text-white/65 leading-relaxed mb-7" />
                            )}
                            ...
                            <div className="flex items-center gap-6 mb-7">
                                <div>
                                    <p className="font-lastica text-[8px] tracking-[3px] text-white/50 uppercase mb-3">{t('product_qty')}</p>
                                ...
                                <div className="pt-6">
                                    {inStock
                                        ? <p className="font-glacial text-sm text-green-400">● {t('product_in_stock')} <span className="text-green-400/60">({product.stock})</span></p>
                                        : <p className="font-glacial text-sm text-red-400">● {t('product_out_stock')}</p>
                                    }
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-3 mb-8">
                                <div className="flex flex-col sm:flex-row gap-3">
                                    ...
                                    <button onClick={handleAdd} disabled={!inStock}
                                        ...
                                        {!inStock ? t('product_sold_out')
                                         : added  ? t('product_added')
                                                  : t('product_add')}
                                    </button>
                                    {/* Voir panier */}
                                    <Link to="/panier"
                            ...
                            <div className="pt-6 border-t border-white/[0.08] space-y-4">
                                <p className="font-lastica text-[10px] tracking-[4px] text-white/70 uppercase mb-3">{t('product_info')}</p>
                                {[
                                    { icon: 'M5 13l4 4L19 7', text: t('product_delivery') },
                                    ...
                                    { icon: 'M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z', text: t('product_payment') },
                                ].map((item, i) => (
                    ...
                    {similar.length > 0 && (
                        <section className="mt-24 pt-16 border-t border-white/[0.07]">
                            <div className="flex items-center gap-6 mb-12">
                                <div>
                                    <p className="font-lastica text-[10px] tracking-[5px] text-white/70 uppercase mb-1">{t('product_similar')}</p>
                                    <h2 className="font-glacial text-2xl text-white uppercase tracking-[4px]">Articles <span style={{ color: GOLD }}>{t('product_similar_sub')}</span></h2>
                                </div>
                                ...
                                {similar.map(p => (
                                    <Link key={p._id} to={`/boutique/${p.slug}`} className="group block">
                                        ...
                                        <div className="space-y-1">
                                            {p.category && <p className="font-lastica text-[10px] tracking-[2px] uppercase" style={{ color: GOLD }}>{p.category}</p>}
                                            <p className="font-glacial text-sm text-white uppercase tracking-[1px] group-hover:text-white transition-colors leading-snug">
                                                {p.name}
                                            </p>
