...
export default function Contact() {
    ...
    const infoItems = [
        { icon: 'M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0zM15 11a3 3 0 11-6 0 3 3 0 016 0z', label: t('contact_address'), value: address },
        { icon: 'M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z', label: t('contact_phone_label'), value: phone, href: `tel:${phone.replace(/\s/g,'')}` },
        { icon: 'M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z', label: t('contact_email_label'), value: email, href: `mailto:${email}` },
    ];
        ...
        <Layout title="Contact">
            ...
            <div className="relative h-[40vh] min-h-[280px] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 text-center px-6">
                    <span className="block font-lastica text-[10px] tracking-[6px] text-gold uppercase mb-5" style={{ opacity: 0, animation: 'fadeUp .8s .3s forwards' }}>{t('contact_eyebrow')}</span>
                    <h1 className="font-glacial font-light text-white uppercase tracking-[6px] mb-5" style={{ fontSize: 'clamp(2.4rem,6vw,5rem)', opacity: 0, animation: 'fadeUp .9s .5s forwards' }}>
                        {t('contact_title').split(' ').slice(0,1).join(' ')} <span className="text-gold">{t('contact_title').split(' ').slice(1).join(' ')}</span>
                    </h1>
                ...
                <div className="max-w-7xl mx-auto px-6 lg:px-10">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 sm:gap-16">
                        ...
                        <div className="reveal">
                            <span className="block font-lastica text-[9px] tracking-[5px] text-gold uppercase mb-3">{t('contact_form_title')}</span>
                            <h2 className="font-glacial text-2xl sm:text-3xl font-light text-white uppercase tracking-[2px] sm:tracking-[3px] mb-6 sm:mb-8">{t('contact_form_title')}</h2>
                            {success && (
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <input type="text" placeholder={t('contact_name')} value={form.name} onChange={e => set('name', e.target.value)} required
                                    ...
                                    <div>
                                        <input type="email" placeholder={t('contact_email')} value={form.email} onChange={e => set('email', e.target.value)} required
                                               className="w-full bg-transparent border border-white/25 text-white placeholder-white/35 px-5 py-4 font-glacial text-sm tracking-wide outline-none focus:border-gold/50 transition-colors" />
                                        {errors.email && <p className="text-red-400 text-xs mt-1 font-glacial">{errors.email}</p>}
                                    </div>
                                </div>
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <input type="tel" placeholder={t('contact_phone')} value={form.phone} onChange={e => set('phone', e.target.value)}
                                           className="w-full bg-transparent border border-white/25 text-white placeholder-white/35 px-5 py-4 font-glacial text-sm tracking-wide outline-none focus:border-gold/50 transition-colors" />
                                    <div>
                                        <input type="text" placeholder={t('contact_subject')} value={form.subject} onChange={e => set('subject', e.target.value)} required
                                               className="w-full bg-transparent border border-white/25 text-white placeholder-white/35 px-5 py-4 font-glacial text-sm tracking-wide outline-none focus:border-gold/50 transition-colors" />
                                        {errors.subject && <p className="text-red-400 text-xs mt-1 font-glacial">{errors.subject}</p>}
                                    </div>
                                </div>
                                <div>
                                    <textarea rows={6} placeholder={t('contact_message')} value={form.message} onChange={e => set('message', e.target.value)} required
                        ...
                        <div className="reveal" style={{ transitionDelay: '.15s' }}>
                            <span className="block font-lastica text-[9px] tracking-[5px] text-gold uppercase mb-3">{t('contact_coords')}</span>
                            <h2 className="font-glacial text-3xl font-light text-white uppercase tracking-[3px] mb-8">{t('contact_find')}</h2>
