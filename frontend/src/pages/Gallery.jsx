        ...
        <Layout title="Galerie">
            ...
            <div className="relative h-[40vh] min-h-[280px] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 text-center px-6">
                    <span className="eyebrow justify-center" style={{ opacity: 0, animation: 'fadeUp .8s .3s forwards' }}>{t('gallery_eyebrow')}</span>
                    <h1 className="display-title text-white mt-4" style={{ fontSize: 'clamp(2.4rem,6vw,5rem)', opacity: 0, animation: 'fadeUp .9s .5s forwards' }}>
                        {t('gallery_title').split(' ').slice(0,1).join(' ')} <span className="text-gold">{t('gallery_title').split(' ').slice(1).join(' ')}</span>
                    </h1>
                </div>
            </div>
            ...
            {data.brands.length > 0 && (
                <div className="bg-[#080808] border-b border-gold/8 py-5">
                    <div className="max-w-7xl mx-auto px-6 lg:px-10 flex flex-wrap gap-2.5 items-center">
                        <span className="font-lastica text-[7px] tracking-[4px] text-gold/35 uppercase mr-2">{t('gallery_brands')}</span>
                        {[{ _id: '', name: t('gallery_all') }, ...data.brands].map(b => (
                ...
                <div className="max-w-7xl mx-auto px-6 lg:px-10">
                    {data.items.length > 0 ? (
                        ...
                        <div className="text-center py-24">
                            ...
                            <p className="font-glacial text-sm text-white/55 tracking-[3px] uppercase">{t('gallery_empty')}</p>
                        </div>
                    )}
                </div>
            </section>
