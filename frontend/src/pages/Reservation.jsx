        ...
        <Layout title="Réservation">
            ...
            <div className="relative h-[40vh] min-h-[280px] flex items-center justify-center overflow-hidden">
                ...
                <div className="relative z-10 text-center px-6">
                    <span className="eyebrow justify-center" style={{ opacity: 0, animation: 'fadeUp .8s .3s forwards' }}>{t('reservation_eyebrow')}</span>
                ...
                <div className="max-w-3xl mx-auto px-6 lg:px-10">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6 reveal">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label className="input-label">{t('reservation_name')}</label>
                                <input type="text" value={form.name} onChange={e => set('name', e.target.value)} placeholder={t('reservation_name').replace(' *','')} className="input-mia" required />
                                {errors.name && <p className="text-red-400 text-xs mt-1 font-glacial">{errors.name}</p>}
                            </div>
                            <div>
                                <label className="input-label">{t('reservation_email')}</label>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select value={form.service} onChange={e => set('service', e.target.value)} required
                                    ...
                                    <option value="">{t('reservation_service_ph')}</option>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                            <div>
                                <label className="input-label">{t('reservation_date')}</label>
                                <input type="date" value={form.preferred_date} onChange={e => set('preferred_date', e.target.value)} className="input-mia" />
                            </div>
                            <div>
                                <label className="input-label">{t('reservation_time')}</label>
                                <input type="time" value={form.preferred_time} onChange={e => set('preferred_time', e.target.value)} className="input-mia" />
                            </div>
                        </div>
                        <div>
                            <label className="input-label">{t('reservation_message')}</label>
                            <textarea value={form.message} onChange={e => set('message', e.target.value)} rows={5} placeholder={t('reservation_msg_ph')} className="input-mia resize-none" />
                        </div>
                        <button type="submit" disabled={processing}
                                className="btn btn-gold w-full justify-center disabled:opacity-40">
                            {processing ? t('reservation_sending') : t('reservation_confirm')}
                        </button>
                    </form>
                </div>
            </section>
